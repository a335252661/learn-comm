...
public class MultiThreadServer2 {
    ...
    static class Worker implements  Runnable{
        ...
        public void register(SocketChannel socketChannel) throws Exception{
            ...
            //把这行代码加入到队列中去，在真正的worker 线程中来执行
            // 当第二个客户端连接时候，也需要注册事件，所以放在if外面
            queue.add(() -> {
                try {
                    log.debug("加入队列。。。。。。。。。。。。。。。。");
        ...
        public void run() {
            ...
            while (true){
                try {
                    ...
                    if(task != null){
                        log.debug("task.run(); before..................");
                        task.run();  //真正执行了  socketChannel.register(selector , SelectionKey.OP_READ); 方法
                        log.debug("task.run(); after..................");
                    }
